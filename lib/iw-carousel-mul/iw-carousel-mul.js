document.addEventListener('click', (event) => {
    ...
    if (target.matches('.iw-carousel-mul__control--prev'))
        shiftCarouselMul(target.closest('.iw-carousel-mul'), -1);
    else if (target.matches('.iw-carousel-mul__control--next'))
        shiftCarouselMul(target.closest('.iw-carousel-mul'), +1);
});
/* Shifts the given iw-carousel-mul to the left (if dir == -1) or to the right (if dir == 1). */
function shiftCarouselMul(carousel, dir) {
    const imagesList = carousel.querySelector('.iw-carousel-mul__inner ul');
...
//# sourceMappingURL=iw-carousel-mul.js.map