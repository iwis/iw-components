 ...
 */
function addRelation_RequiredField_LabelAsterisk(field) {
    let label;
    if (field instanceof HTMLInputElement && field.type === 'radio') { // for <input type="radio" required>
        if (!field.closest('.iw-form__check'))
            return; // only for fields from the iw-components library
...
onDOMNodeAdded((node) => {
    if (node instanceof HTMLElement && node.matches('.iw-form :required'))
        addRelation_RequiredField_LabelAsterisk(node); // <input>, <textarea> or <select> field
...
document.addEventListener('change', (e) => { ifFieldsetCheckboxThenValidateFieldset(e.target); }); // a checkbox was checked or unchecked
...
function ifFieldsetCheckboxThenValidateFieldset(node) {
...
function validateFieldset(fieldset) {
    ...
    checkboxes = fieldset.querySelectorAll('.iw-form__check input[type="checkbox"]'), numOfChecked = fieldset.querySelectorAll('.iw-form__check input[type="checkbox"]:checked').length;
    let validationMessage = ''; // todo: can be translated to other languages
    if (min && numOfChecked < Number(min))
        switch (navigator.language.substring(0, 2)) {
            case 'pl':
                validationMessage = `Wybierz przynajmniej       ${min}.`;
                break;
    ...
    if (max && numOfChecked > Number(max))
        switch (navigator.language.substring(0, 2)) {
            case 'pl':
                validationMessage = `Wybierz nie więcej niż     ${max}.`;
                break;
            default: validationMessage = `Please select no more than ${max} checkbox(es).`;
        }
    checkboxes.forEach((checkbox) => checkbox.setCustomValidity(validationMessage));
}
//# sourceMappingURL=iw-form.js.map