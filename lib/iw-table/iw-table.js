...
function areInOneTable(element1, element2) {
...
document.addEventListener('dragenter', (e) => {
    if (e.target instanceof HTMLTableCellElement && e.target.matches('.iw-table--draggable-cols thead th')) {
        ...
        if (areInOneTable(iwTableGrabbedColumn, column)) { // only column of the same table can be dropped
            if (iwTableGrabbedColumn.cellIndex < column.cellIndex)
                column.style.borderRight = '2px solid gray';
            else if (column.cellIndex < iwTableGrabbedColumn.cellIndex)
                column.style.borderLeft = '2px solid gray';
            else
                ; // no border
        }
    }
    ...
    if (target.closest('.iw-table--draggable-cols thead th') !== null) {
        ...
        if (UI_MODE === 'IMMEDIATE') { // ------------------------------
            if (e.key === ' ') {
                ...
                if (currentColumn.cellIndex + direction < 0 || currentColumn.cellIndex + direction >= columnsNumber)
                    return;
            ...
            if (iwTableGrabbedColumn === undefined && e.key === ' ') { // 1. GRAB current column (i.e. select for dragging)
                ...
                if (iwTableGrabbedColumn) {
                    if (areInOneTable(iwTableGrabbedColumn, currentColumn)) { // only column of the same table can be dropped
                        ...
                        if (iwTableGrabbedColumn.cellIndex < currentColumn.cellIndex)
                            currentColumn.style.borderRight = '2px solid gray';
                        else if (currentColumn.cellIndex < iwTableGrabbedColumn.cellIndex)
                            currentColumn.style.borderLeft = '2px solid gray';
                        else
                            ; // no border
...
//# sourceMappingURL=iw-table.js.map