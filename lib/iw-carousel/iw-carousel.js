...
document.addEventListener('click', (event) => {
    ...
    if (target.matches('.iw-carousel__control--prev'))
        shiftCarousel(target.closest('.iw-carousel'), -1);
    else if (target.matches('.iw-carousel__control--next'))
        shiftCarousel(target.closest('.iw-carousel'), +1);
    else if (target.matches('.iw-carousel__indicators li'))
        showCarouselSlide(target.closest('.iw-carousel'), Number(target.dataset.slideTo));
});
...
//# sourceMappingURL=iw-carousel.js.map