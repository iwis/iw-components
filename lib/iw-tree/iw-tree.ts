...
document.addEventListener('keydown', (event) => {
  ...
  if (target.classList.contains('iw-tree__node--inner') ||
    ...
    if (event.key === 'ArrowUp')   { event.preventDefault(); focusNextElementOf(target.closest('.iw-tree'), -1) }
    if (event.key === 'ArrowDown') { event.preventDefault(); focusNextElementOf(target.closest('.iw-tree'),  1) }
  }
...
function toggleTreeNode(innerNode: HTMLElement) {
...
export function openAllTreeNodes(tree: HTMLElement) {
