...
export function addClassAndRemoveInSiblings(element, CSSClass) {
...
export function setAttributeAndRemoveInSiblings(element, name, value) {
    for (const sibling of element.parentNode.children)
 ...
 */
export function onDOMNodeAdded(callback) {
 ...
 */
export function onDOMChange(callback) {
 ...
 */
export function isFullyInViewport(el) {
 ...
 */
export function objectTableToHTMLTable(objects, cssClass = '') {
    // table
 ...
 */
export function moveHTMLTableColumn(table, column, toColumn) {
    const columnsNumber = table.rows[0].cells.length;
    if (column < 0 || column >= columnsNumber)
        throw new RangeError('column argument must belong to <0, number_of_columns)');
    if (toColumn < 0 || toColumn >= columnsNumber)
        throw new RangeError('toColumn argument must belong to <0, number_of_columns)');
    for (const row of table.rows) {
        if (column < toColumn)
            row.cells[toColumn].after(row.cells[column]);
        else if (toColumn < column)
            row.cells[toColumn].before(row.cells[column]);
    }
}
 ...
 */
export function sortHTMLTable(table, column, ascending = true, skipTopRows = 0) {
    const th = table.tHead.rows[0].cells[column];
    // function comparing 2 rows of the table
    let compare; // type of function in TypeScript
...
*/
export function createTreeHTML(obj) {
    let result = '';
...
export function createTreeDOM(obj) {
 ...
 */
export function addControlsCSSClassesAndARIAToTree(tree) {
 ...
 */
export function positionAt(anchor, elem, placement, stickToWindow = false, space = 2) {
    ...
    if (main === 'top' || main === 'bottom') {
        ...
        if (aux === 'start')
            ...
            left = anchorCoords.left + (anchor.offsetWidth - elem.offsetWidth) / 2;
        else if (aux === 'end')
            left = anchorCoords.right - elem.offsetWidth;
        else
            throw new Error('Incorrect value of "placement" parameter');
        ...
        if (main === 'left')
            left = anchorCoords.left - elem.offsetWidth - space;
        else if (main === 'right')
            left = anchorCoords.right + space;
        if (aux === 'start')
            top = anchorCoords.top;
        else if (aux === undefined)
            top = anchorCoords.top + (anchor.offsetHeight - elem.offsetHeight) / 2;
        else if (aux === 'end')
            top = anchorCoords.bottom - elem.offsetHeight;
        else
            throw new Error('Incorrect value of "placement" parameter');
    }
    else
 ...
 */
export function focusNextElementOf(mainElement, direction = 1) {
 ...
 */
export function focusNextSiblingOf(element, direction = 1) {
    if (document.activeElement) {
 ...
 */
export function eduGetElementsByTagName(node, tagName) {
    let found = [];
...
//# sourceMappingURL=iw-browser.js.map